...
function productCard(p) {
  ...
  const realImg  = p.img;
  const localImg = 'http://localhost:3000/pimg/' + p.id;
  const parts = [];
    ...
    <div class="cart-card">
      ...
      ${cart.length===0 ? '<div class="empty-cart"><div style="font-size:3rem">🛒</div><p style="margin-top:12px;font-weight:800">Sua sacola está vazia</p><a href="/" style="color:var(--blue);font-weight:700;margin-top:8px;display:inline-block">Continuar comprando →</a></div>' :
        ...
        <div class="ci-row" id="ci-${i}">
          <img class="ci-img" src="http://localhost:3000/img?url=${encodeURIComponent(item.img)}" alt="${item.name}" onerror="this.src='https://placehold.co/80x80?text=?'"/>
          <div class="ci-info">
...
const server = http.createServer(async (req, res) => {
  ...
  if (method==='POST' && p==='/api/checkout') {
    ...
    const order    = {id:orderId,userToken:tk,items:[...cart],total,status:'pending',createdAt:new Date().toISOString(),paymentLink:`http://localhost:3000/pay/${orderId}`};
    orders.push(order);
    carts[tk] = [];
